...
{
    info->request_handler(info->server, info->request);
    free(info->request.buffer); // Звільняємо буфер у разі помилки
    free(info);
}

// Функція для запуску сервера
void udp_server_listen(UdpServer *server, RequestHandler handler, threadpool pool) // Можна тільки в одному потоці
{
    ...
    request->packet_size = recvfrom(server->sockfd, request->buffer, server->buffer_size, 0, (struct sockaddr *)&request->client_addr, &addr_len);

    if (request->packet_size > 0)
    {
        thpool_add_work(pool, (void (*)(void*))&_udp_server_request_handler, (void *)rinfo);
        rinfo = NULL;
    }
